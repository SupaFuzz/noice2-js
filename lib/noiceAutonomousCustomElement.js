...
*/
import { isNull, isNotNull, epochTimestamp } from './noiceCore.js';
...
initialize(){
    ...
    // also dispatch the initialized event
...
/*
    log(string | object)
    if you send an object it MUST contain the 'message' key at a minumum
    if you send a string you're sending the content of the 'message' key on the resulting object
    if this.debug is set hot, we'll console.log the 'message'
    then we will call the log event on self with a detail object thusly: detail {
        time: epochTimestamp(true),
        self: this,
        message: <str>
    }
    if you sent a whole object in, we're just gonna Object.assign that struct
    with what you sent.
*/
log(inp){
    let that = this;
    let out = Object.assign({ time: epochTimestamp(true), message: `${that._className} v${that._version} | ${inp}`}, (inp instanceof Object)?inp:{});
    out.self = that;
    if (that.debug === true){ console.log(`[${out.time}] ${out.message}`); }
    that.dispatchEvent(new CustomEvent("log", { detail: out }));
}
set logCallback(v){ if (v instanceof Function){
    this.addEventListener('log', (evt) => { v(evt.detail); });
}}
