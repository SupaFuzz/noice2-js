...
/*
    ...
    if replaceOK is true (the default), if the specified file already exists
    we're gonna just do the lazy thing and just blow away the whole row on
        table: `${args.hasOwnProperty('schemaName')?args.schemaName:'mezo'}.files`,
    because I guess "files" is the hard-coded table name and the schema just changes
    or whatever, 'cause thats how I originally wrote it.

    if you set replaceOK false, and it exists already we're gonna boot it back to ya with an error
*/
putFile(args){
    let that = this;
        ...
        if (
            ...
            new Promise((_t,_b) => {
                ...
                that.getFile({
                    ...
                    schemaName: args.hasOwnProperty('dbSchema')?args.dbSchema:'mezo'
                }).then((fileExistsResponse) => {
                    if (args.hasOwnProperty('replaceOK') && (args.replaceOK === false)){
                        _b(new noiceException({
                            messageNumber: 300,
                            messageType: 'internal',
                            message: `file already exists and 'replaceOK' flag is not set true, exiting`,
                            thrownBy: `${that._className} v${that._version} | putFile()`,
                            args: {fileName: args.fileName, dbSchema: args.hasOwnProperty('dbSchema')?args.dbSchema:'mezo', replaceOK: false }
                        }));
                    }else{
                        // delete that sucka!
                        that.deleteRow({
                            table: `${args.hasOwnProperty('dbSchema')?args.dbSchema:'mezo'}.files`,
                            id: fileExistsResponse.id
                        }).then(() => {
                            _t(true);
                        }).catch((error) => {
                            error.message = `failed to delete existing row (id: ${fileExistsResponse.id}) on ${args.hasOwnProperty('dbSchema')?args.dbSchema:'mezo'}.files | ${error.message}`;
                            _b(error);
                        });
                    }
                }).catch((error) => {

                    // if it's 404, cool if not thats an issue
                    if (
                        (error instanceof Object) &&
                        (error.errorObject instanceof Object) &&
                        (error.errorObject.hasOwnProperty('messageNumber')) &&
                        (parseInt(error.errorObject.messageNumber) == 404)
                    ){
                        _t(false);
                    }else{
                        // really barf
                        error.message = `fatal error getting meta for existing file: ${error.message}`;
                        _b(error);
                    }
                })
            }).then(() => {
    ...
    return(new Promise((toot, boot) => {
        if (
            ...
            if ( args.hasOwnProperty('getData') && (args.getData === false) ){
                ...
                this.getRows({
                    table: `${args.hasOwnProperty('schemaName')?args.schemaName:'mezo'}.files`,
                    field_list: ['id', 'type', 'name', 'modified_date', 'size'],
