...
signalParent(args){
    ...
    */
    ...
    if (
        ...
        /*
            2/27/26 @ 1315 -- ok so there's a real problem here that sometimes
            things send events with uncloneable objects burried in them (for instance an xhr ref)
            and THIS causes the wholle thread to fall over and crash.

            so here's what we're gonna do.
            we're gonna try twice. if it fails the first time we're gonna squash it with JSON.stringify
            and try again. If that doesn't work, we go ahead and throw it
        */
        ...
        try {
            ...
            // funk dat
            try {
                this.threadHandle.postMessage(JSON.parse(JSON.stringify(args)));
            }catch(er){
                // okay I guess we really can't
                console.log(`[${this.threadName}] signalParent(threadHandle) | postMessage() failed (hard): ${er}`, args);
            }
        ...
        try {
            ...
            try {
                this.broadcastChannel.postMessage(JSON.parse(JSON.stringify(args)));
            }catch(e){
                console.log(`[${this.threadName}] signalParent(broadcastChannel) | postMessage() failed (hard): ${er}`, args);
            }
        }
    }else{
