                            ...
                            new Promise((_t, _b) => {
                                if (args.attachments instanceof Object){
                                    let putQueue = Object.keys(args.attachments).filter((fieldName) => {return(
                                        ...
                                        data: args.attachments[fieldName].content,
...
/*
    ...
    direct emulation of noiceRemedyAPI.modifyTicket
    not gonna lie, I just copied createTicket because the only thing different
    is that we have a ticketNumber to post it to <lady-shrug-emoji>
        ...
        if (
            ...
            if (that.isAuthenticated){
                ...
                Promise.resolve((args.formDefinition instanceof Object)?args.formDefinition:that.getFormFields({schema: args.schema})).then((formDefinition) => {
                    ...
                    // modify the row (if we had attachments, we'll add them later)
                    that.modifyRow(callArgs).then((resp) => {


                        // I feel cheeky. Oh so cheeky
                        Promise.resolve({
                            '_1': args.ticket
                        }).then((r) => {
                            ...
                            */
                            new Promise((_t, _b) => {
                                ...
                                // emulate noiceRemedyAPI modifyTicket() response
                                toot({
                                    url: `${that.protocol}://${that.server}:${that.port}/${that.isNotNull(that.proxyPath)?that.proxyPath:''}/${formDefinition.table_name}?_1=eq.${r._1}`,
                                    entryId: r._1
                                });
                            ...
                            boot(new noiceException ({
                                messageNumber:          501,
                                messageType:            'server',
                                message:                `this should be technically impossible to reach`,
                                thrownByFunction:       fn,
                                thrownByFunctionArgs:   args,
                                errorObject:            error
                            }));
                        ...
                        boot(new noiceException ({
                            messageNumber:          501,
                            messageType:            'server',
                            message:                `modifyRow() threw unexpectedly ${error}`,
                            thrownByFunction:       fn,
                            thrownByFunctionArgs:   args,
                            errorObject:            error
                        }));
            ...
            if (that.isAuthenticated){
                ...
                Promise.resolve((args.formDefinition instanceof Object)?args.formDefinition:that.getFormFields({schema: args.schema})).then((formDefinition) => {
                    ...
                    if (args.fields instanceof Array){
                        callArgs.field_list = args.fields.filter((fieldName) => {return(
                        ...
                        )}).map((fieldName) => {return( `_${formDefinition.nameIndex[fieldName].id}` )}).concat(
                            args.fields.filter((fieldName) => {return(
                                (! (
                                    ...
                                    (that._openAPIDefinitions instanceof Object) &&
                                    (that._openAPIDefinitions[formDefinition.table_name] instanceof Object) &&
                                    (that._openAPIDefinitions[formDefinition.table_name].properties instanceof Object) &&
                                    (that._openAPIDefinitions[formDefinition.table_name].properties[fieldName] instanceof Object)
                                )
                    ...
                    that.getRow(callArgs).then((row) => {
                        ...
                        // new hotness includes requested columns not mapped to fields in emulated form
                        let out = { values: {}};
                        Object.keys(row).forEach((colName) => {
                            ...
                            if (
                                ...
                                useColName = formDefinition.idIndex[colName.replace('_','')].name;
                            }
                            out.values[useColName] = row[colName];
                        });
