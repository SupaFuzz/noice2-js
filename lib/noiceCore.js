        ...
        */
        ...
        xhr.addEventListener("load", function(){
            if (myOKStatuses.indexOf(this.status) >= 0){
                if (self.fetchSuccessMonitor instanceof Function){ self.fetchSuccessMonitor(this.status); }
                resolve(this);
            }else{
                abort = true;
                const err = new noiceException({
                    message:        `received unexpected HTTP status ${this.status}, expected ${myOKStatuses.join(", OR ")}`,
                    messageNumber:   10,
                    messageType: /^5\d\d/.test(this.status)?'server':'',
                ...
                if (self.fetchErrorMonitor instanceof Function){ self.fetchErrorMonitor(err); }
                reject(err);
            }
        });
    ...
    return(new Promise(function(toot, boot){
        ...
        fetch(args.endpoint, fetchArgs).then((response) => {
            ...
            if (
                ...
                if (/^5\d\d/.test(`${response.status}`)){ errorArgs.messageType = "server"; }
                response.json().catch(function(error){
                    ...
                    if (that.fetchErrorMonitor instanceof Function){ that.fetchErrorMonitor(new noiceException(errorArgs)); }
                ...
                */
                ...
                let reader = null;
                /*
                    node causes problems without this.
                    I do not compeletely understand the reason but it works so ok
                */
                try {
                    reader = response.body.getReader();
                }catch(e){
                    //console.log(`well ok then! ${e}`);
                }
                const chunks = [];
                let receivedBytes = 0;
                    ...
                    reader.read().then((dta) => {
                        ...
                        if (dta.done == true){
                            ...
                            switch(args.responseType){
                                case 'text':
                                    if (that.fetchSuccessMonitor instanceof Function){ that.fetchSuccessMonitor(); }
                        ...
                        if (that.fetchErrorMonitor instanceof Function){ that.fetchErrorMonitor(err); }
                        boot(err);
                    })
                }
                /*
                    NOTE: 10/27/25 -- I do not understand why this is necessary
                    except that deleteRow() on noicePGRestAPI (a headers-only request)
                    inside node -- this will throw an uncaught exception if not for the check
                */
                if (that.isNotNull(reader)){
                    recursor(reader);
                }else{
                    if (self.fetchSuccessMonitor instanceof Function){ self.fetchSuccessMonitor(); }
                    toot({});
                }
...
const distinctCombinations = noiceCoreUtility.prototype.distinctCombinations;
const segmentArray = noiceCoreUtility.prototype.segmentArray;
