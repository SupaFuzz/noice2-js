    ...
    return(new Promise((toot, boot) => {
        ...
        /*
            ...
            we gotta filter invalid _bar's:
                ...
                * orphanse that have a parent_name attribute that does not exist as a 'name' in ._bars
        */
        ...
        let oof_counter = 0;
        function recursor(){
            const parents = Array.from(that.svgDOMObject.querySelectorAll('rect'));
            ...
            if (q.length > 0){
                q.forEach((b, idx) => {
                    ...
                    if (!((b.hasOwnProperty('fill') && that.isNotNull(b.fill)))){ bar.classList.add("bar"); }
                ...
                toot(Array.from(that.svgDOMObject.querySelectorAll('rect')));
            }
        }
        recursor();
    }));
