            ...
            function recursor(){
                ...
                if (q.length > 0){
                    q.forEach((b, idx) => {
                        ...
                        ['value', 'scaled_value', 'name', 'order', 'label', 'enable_hover', 'highlight_label'].filter((at) => {return(b.hasOwnProperty(at))}).forEach((a) => { bar.dataset[a] = b[a]; });
     ...
     if (that.initialized){
         ...
         if (bool === true){
             ...
             bars.filter((el) => {return(
                 ...
                 div.innerHTML = `<span ${(bar.dataset && bar.dataset.highlight_label && (bar.dataset.highlight_label == "true"))?'class="highlight"':''}>${(bar.dataset && bar.dataset.label && that.isNotNull(bar.dataset.label))?bar.dataset.label:''}</span>`;
                 txt.appendChild(div);
    ...
    if (
        ...
        if (chk.length == 0){
            if (args.click_callback instanceof Function){
                ...
                this.addEventListener(`bar_click_${args.name}`, (evt) => { ref(that, evt.detail.self, args.name); });
            }
...
get defaultStyle(){return(`
    ...
    foreignObject.label div.textLabel span.highlight {
