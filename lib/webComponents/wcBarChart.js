...
static classAttributeDefaults = {
    ...
    enable_label_rotate: { observed: true, accessor: true, type: 'bool', value: true, forceAttribute: true },
}
...
static classStyleDefaults = {
    ...
    'base-layer-highlight-color': { value: 'hsl(233 6% 56.1%)', global: true }
}
    ...
    return(new Promise((toot, boot) => {
        if (that.svgDOMObject instanceof Element){
            ...
            */
            ...
            // handle data scaling
            if ((queue.length > 0) && (that.scale_values == true)){
                const bs = queue.slice();
                bs.sort((a,b) => {return( parseFloat(b.value) - parseFloat(a.value) )});
                let hwm = bs[0].value;
                queue.forEach((bar) => {
                    if (
                        bar.hasOwnProperty('value_relative_to_parent') &&
                        (bar.value_relative_to_parent == true) &&
                        bar.hasOwnProperty('parent_name')
                    ){
                        let p = queue.filter((a) => {return(a.name == bar.parent_name)});
                        if (p.length > 0){
                            if (p[0].hasOwnProperty('scaled_value')){
                                bar.scaled_value = ((parseFloat(bar.value) / parseFloat(p[0].scaled_value)) *100);
                            }else{
                                bar.scaled_value = ((parseFloat(bar.value) / ((parseFloat(p[0].value) / parseFloat(hwm)) *100)) *100);
                            }
                        }
                    }else{
...
/*
    addBar({
        ...
        value_relative_to_parent: <bool> if true, and if scaled_values turned on, scale value relative to parent rather than hwm
    })
...
getBar(name){
    if (this.isNotNull(name)){
        const b = this._bars.filter((a) => {return(a.name == name)});
        if (b.length > 0){
            const el = this.svgDOMObject.querySelector(`rect.bar[data-name="${name}"]`);
            if (el instanceof Element){
                return(Object.assign({}, b[0], {DOMElement: el}));
            }
        }
    }
}
...
get defaultStyle(){return(`
    ...
    rect.bar.base_layer[data-highlight_label="true"] {
        fill: ${this.styleVar('base-layer-highlight-color')};
    }
    rect.bar.base_layer {
    ...
    :host([enable_label_rotate="true"]) foreignObject.label div.textLabel span {
