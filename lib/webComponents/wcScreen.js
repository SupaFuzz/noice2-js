...
constructor(args){
    ...
    this.attributeChangeHandlers = {
        ...
        disabled: (a,o,n,s) => {
            // just dispatch the event so it can be caught by subclasses
            that.dispatchEvent(new CustomEvent("disabled", { detail: { screen: that, disabled: (n === true) }}));
        },
    ...
    return(new Promise((toot, boot) => {
        if ((focusBool == true) && (that.gainFocusCallback instanceof Function)){
            ...
            that.gainFocusCallback(focusArgs, that).then((resp) => {
                ...
                that.dispatchEvent(new CustomEvent("focus_error", { detail: { focus: true, screen: that, focusError: error }}));
                boot(error);
            });

        }else if ((focusBool == false) && (that.loseFocusCallback instanceof Function)){

            that.loseFocusCallback(focusArgs, that).then((resp) => {
