...
static classStyleDefaults = {
    ...
    'menu-button-alert-start-color': { value: 'rgba(240,240,240,.3)', global: true},
    'menu-button-alert-end-color': { value: 'transparent', global: true},
    'menu-button-notice-start-color': { value: 'rgba(110,200,110,.3)', global: true},
    'menu-button-notice-end-color': { value: 'transparent', global: true},
};
...
switchUI(screenName, focusArgs){
    ...
    return(new Promise((toot, boot) => {
        ...
        /*
            default_ui -- if we have one, and screenName is null, replace null with it
        */
        if (that.isNull(screenName)){
            const default_ui_check = Object.keys(that.UIs).filter((uiName) => {return(
                that.UIs[uiName].hasOwnProperty('default_ui') && (that.UIs[uiName].default_ui == true)
            )});
            if (default_ui_check.length > 0){ screenName = default_ui_check[0]; }
        }
        ...
        new Promise((_t) =>{_t(
            ...
            new Promise((_t) =>{_t(
                ...
                if (el instanceof Element){
                    ...
                    that.log(`${that._className} v${that._version} | switchUI(${screenName}) | screenName somehow unknown?`);
                    toot(null);
                }
            }).catch((error) => {
            ...
            that.log(`${that._className} v${that._version} | switchUI(${screenName}) | currentUI.setFocus(false) threw aborting UI change: ${error}`);
            boot(error);
        });
...
/*
    getUIMenu(externalMenuItems)
    externalMenuItems is a list of menu items to merge with the UIs
    it should be an array of objects of this form:
    [
        {
            name: <str>,
            menu_label: <str>,
            menu_order: <str>,
            menu_icon_mask: <svg src>,
            callback: (btnRef) => { ... do stuff ... }
        },
        ...
    ]
*/
getUIMenu(externalMenuItems){
    ...
    return(new wcBasic({
        ...
        initializedCallback: (slf) => {
            ...
            /*
                LOH 1/14/26 @ 1643 COB
                next up: implement externalMenuItems as described above
            */
                ...
                slf.generateMenu = () => {
                    ...
                    slf._elements.menu.innerHTML = Object.keys(that.UIs).map((a) => {return(that.UIs[a])}).concat(
                            (externalMenuItems instanceof Array)?externalMenuItems.map((itm) => {return({dataset: itm})}):[]
                    ).filter((a) => {return(
                        ! ((a.hasOwnProperty('show_in_menu')) && (a.show_in_menu == false))
                    )}).sort((a,b) => {return(
                    ...
                    slf._elements.menu.querySelectorAll('button').forEach((el) => {
                        el.addEventListener("click", (evt) => {
                            ...
                            if (that.UIs.hasOwnProperty(el.dataset._name)){
                                // if it's a ui
                            ...
                            }else if (externalMenuItems instanceof Array){
                                // if it's an externalMenuItems
                                let guh = externalMenuItems.filter((a) => {return(
                                    (a instanceof Object) &&
                                    (a.callback instanceof Function) &&
                                    a.hasOwnProperty('name') &&
                                    (a.name == el.dataset._name)
                                )});
                                if (guh.length > 0){
                                    guh[0].callback(el);
                                }
                    ...
                    // update the css
                    let btn_icons = Object.keys(that.UIs).map((a) => {return(that.UIs[a])}).concat(
                        (externalMenuItems instanceof Array)?externalMenuItems.map((itm) =>{return({
                            dataset: {name: itm.name},
                            menu_icon_mask: itm.hasOwnProperty('menu_icon_mask')?itm.menu_icon_mask:null
                        })}):[]
                    ).filter((a) => {return(
                    ...
                    slf.styleSheet = `
                        ...
                        button[data-alert="true"]{
                            animation: alert 1.5s infinite;
                        }
                        @keyframes alert {
                            0%    { background-color: ${this.styleVar('menu-button-alert-start-color')}; }
                            100%  { background-color: ${this.styleVar('menu-button-alert-end-color')};  }
                        }
                        button[data-notice="true"]{
                            animation: notice 3.5s infinite;
                        }
                        @keyframes notice {
                            0%    { background-color: ${this.styleVar('menu-button-notice-end-color')}; }
                            50%    { background-color: ${this.styleVar('menu-button-notice-start-color')}; }
                            100%  { background-color: ${this.styleVar('menu-button-notice-end-color')};  }
                        }
                ...
                // listen for reset_selections
                slf.addEventListener('reset_selections', (evt) => {
                    slf._elements.menu.querySelectorAll('button').forEach((el) => {
                        el.dataset.selected = (
                            (that.currentUI instanceof Element) &&
                            (that.currentUI.dataset.name == el.dataset._name)
                        )
                    })
                });
            }
