...
import { epochTimestamp } from '../noiceCore.js';
...
static classAttributeDefaults = {
    ...
    show_alert_menu: { observed: true, accessor: true, type: 'bool', value: false, forceAttribute: true },
...
static classStyleDefaults = {
    ...
    'header-button-size': { value: '1.5rem', global: true },
    'header-button-margin': { value: '.5rem', global: true },
    'alert-hover-color': { value: 'rgba(191, 191, 24, .1)', global: true},
...
constructor(args){
    ...
    this.mergeClassDefaults({
        ...
        _statusTimeout: (5 * 1000), // 15 second timeout on stuck indicator stuff
        _lastStatus: 0,
        _alertMenuItems: {}
    });
...
openBurgerMenu(bool){
    ...
    if (this.initialized && (this.burgerMenu instanceof wcBalloonDialog)){
        if (bool === true){
            ...
            this._elements.btnBurger.disabled = true;
...
openProgressMenu(bool){
    ...
    if (this.initialized && (this.progressMenu instanceof wcBalloonDialog)){
        if (bool === true){
            ...
            that.dispatchEvent(new CustomEvent("open_dialog", { detail: {self: that, dialog_name: 'progress_menu', ui: that.progressMenu.firstChild }}));
        }else{
    ...
    div.insertAdjacentHTML('afterbegin', `
        <div data-_name="header">
            ...
            <div data-_name="btnContainer">
                ...
                <button data-_name="btnAlert"></button>
                <slot name="custom_button"></slot>
            </div>
        </div>
...
initializedCallback(){
    ...
    this.burgerMenu = new wcBalloonDialog({
        ...
        exitCallback: async (slf) => {
            ...
            this._elements.btnBurger.disabled = false;
            return(false);
        },
    });
    ...
    // hook for btnAlert
    this._elements.btnAlert.addEventListener('click', (evt) => {that.openAlertMenu(true); });
    ...
    // every _statusTimeout, check to see if _lastStatus is older than _statusTimeout and remove it if so
    that._statusTimer = setInterval(() => {
        if (
            (that._elements instanceof Object) &&
            (that._elements.btnIndicator instanceof Element) &&
            that._elements.btnIndicator.dataset.status &&
            that.isNotNull(that._elements.btnIndicator.dataset.status) &&
            ((epochTimestamp(true) - that._lastStatus) > that._statusTimeout)
        ){
            that._elements.btnIndicator.dataset.status = '';
            that._lastStatus = 0;
        }
...
yoinkUm(){
    ...
    // don't full on yoink um, but catch custom_buttons and force local style vars upon them
    Array.from(that.children).filter((el) => {return(el.slot == "custom_button")}).forEach((el) => {
        el.style.height = `${this.styleVar('header-button-size')}`;
        el.style.width = `${this.styleVar('header-button-size')}`;
        el.style.margin = `${this.styleVar('header-button-margin')}`;
        el.style.border = 'none';

        if (el.tagName == 'BUTTON'){
            // note: set your own 'mask' externally to get the nice icon, etc
            el.style.backgroundColor = `${this.styleVar('header-color')}`;
...
/*
    ...
    if lightbox set true, set the lightbox option on the balloon dialog
*/
openPanel(ui, title, type, ui_name, lightbox){
    let that = this;
    if (ui instanceof Element){

        // popadilly modal
        if (that.panelDilly instanceof wcBalloonDialog){ that.panelDilly.remove(); }
...
get defaultStyle(){return(`
    ...
    :host([show_alert_menu="false"]) button[data-_name="btnAlert"],
    ...
    div[data-_name="header"] div[data-_name="btnContainer"] button {
        height: ${this.styleVar('header-button-size')};
        width: ${this.styleVar('header-button-size')};
        margin: ${this.styleVar('header-button-margin')};
    ...
    button[data-_name="btnAlert"] {
        mask: var(--wc-main-ui-burger-button-mask, url('data:image/svg+xml;utf8,${this.warningIcon}'));
        background-color: ${this.styleVar('header-color')};
        border: none;
        margin-right: 1rem;
    }
...
/*
    setIndicatorStatus(string)
    set this._elements.btnIndicator.dataset.status
    that is all, it's just better to have a function for it
    than to expect external code to dig into the internals
    supported strings ... see the CSS above
*/
setIndicatorStatus(string){
    if ((this._elements instanceof Object) && (this._elements.btnIndicator instanceof Element)){
        this._elements.btnIndicator.dataset.status = string;
        this._lastStatus = epochTimestamp(true);
    }
}
...
/*
    alertMenuItems stuff
    <docs here when I get there>
*/
...
/*
    ...
    this will add the 'item' (a string or element) to this._alertMenuItems under the specified 'id',
    when clicked the menu item will emit the 'event_name' event with the menu item element on
    event.detail.self

    if this is the first alert item added, it will also set show_alert_menu true unhiding the menu icon.

    When openAlertMenu() is called (via click), the ballonDialog
    is generated and put onscreen, with the contents of this._alertMenuItems rendered as menu items
    sorted by the datetime they were added via this function.
*/
addAlertMenuItem(args){
    if (
        (args instanceof Object) &&
        args.hasOwnProperty('id') &&
        this.isNotNull(args.id) &&
        args.hasOwnProperty('item') && (
            (args.item instanceof Element) ||
            this.isNotNull(args.item)
        )
    ){
        // make the element and stash it on _alertMenuItems for openAlertMenu() to render
        const mnu_item = (args.item instanceof Element)?args.item:document.createElement('div');
        if (! (args.item instanceof Element)){ mnu_item.textContent = args.item; }
        mnu_item.classList.add('menuItem');
        mnu_item.dataset.clickable = (args.callback instanceof Function);
        if (args.callback instanceof Function){ mnu_item.addEventListener('click', (evt) => { args.callback(mnu_item); }); }
        this._alertMenuItems[args.id] = Object.assign({}, args, {time: epochTimestamp(true), element: mnu_item, callback: null});
        this.show_alert_menu = true;
    }
}
...
/*
    removeAlertMenuItem(id)
    if the given id exists in this._alertMenuItems delete it (and remove it if it's onscreen)
    if it was the last item, close the dialog (if open) and set show_alert_menu false
*/
removeAlertMenuItem(id){
    if (this._alertMenuItems.hasOwnProperty(id)){
        if (this._alertMenuItems[id].hasOwnProperty('element') && (this._alertMenuItems[id].element instanceof Element)){ this._alertMenuItems[id].element.remove(); }
        delete(this._alertMenuItems[id]);
        this.show_alert_menu = (Object.keys(this._alertMenuItems).length > 0);
        if ((! this.show_alert_menu) && (this.alertMenu instanceof wcBalloonDialog)){ this.alertMenu.exit(); }
    }
}
...
/*
    openAlertMenu()
    create this.alertMenuDialog and render the contents of this._alertMenuItems into
    it as dialogContent, then put it on the screen
*/
openAlertMenu(bool){
    let that = this;
    if (this.initialized){
        if (bool === true){
            ...
            // spawn the alert menu
            this.alertMenu = new wcBalloonDialog({
                arrow_position: 'topRight',
                alignment_mode: 'topRight',
                title: '',
                exitCallback: async (slf) => {
                    that.dispatchEvent(new CustomEvent("close_dialog", { detail: {self: that, dialog_name: 'alert_menu' }}));
                    that._elements.btnAlert.disabled = false;
                    return(false);
                },
            });
            this.alertMenu.relativeElement = this._elements.btnAlert;

            // plop everything in _alertMenuItems into a wcBasic, and make that the dialogContent
            const menu = new wcBasic({
                content: `<div data-_name="alertMenu"></div>`,
                styleSheet: `
                    .menuItem {
                        margin: 0;
                        padding: .25rem .75rem .25rem .75rem;
                        color: ${that.styleVar('header-color')};
                    }
                    .menuItem:nth-child(n+2){
                        border-top: 1px solid ${that.styleVar('header-color')};
                    }
                    .menuItem[data-clickable="true"]:hover {
                        background-color: ${that.styleVar('alert-hover-color')};
                    }
                `,
                initializedCallback:(slf) => {
                    Object.keys(that._alertMenuItems).sort((a,b) => {return(b.time - a.time)}).map((a)=>{return(that._alertMenuItems[a])}).forEach((itm) => {
                        slf._elements.alertMenu.appendChild(itm.element);
                    });
                }
            });

            menu.setAttribute('slot', 'dialogContent');
            this.alertMenu.appendChild(menu);

            // put it on screen, disable the button that opened it and send the open_dialog event
            this.DOMElement.appendChild(this.alertMenu);
            this._elements.btnAlert.disabled = true;
            that.dispatchEvent(new CustomEvent("open_dialog", { detail: {self: that, dialog_name: 'alert_menu', ui: that.alertMenu.firstChild }}));
        }else{
            this.alertMenu.exit();
        }
    }
