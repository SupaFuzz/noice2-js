...
static classAttributeDefaults = {
    ...
    sort_col_selector_alpha: { observed: true, accessor: true, type: 'bool', value: false },
...
get defaultStyle(){return(`
    ...
    .footer .buttonContainer button.txtBtn[data-inv="true"],
    .footer .buttonContainer button.txtBtn[data-inv_on_enable="true"]:not(:disabled)
    {
    ...
    if (v instanceof Array){
        ...
        if (that.initialized){
            ...
            v.filter((a)=>{return(
                ...
                btn.dataset.inv_on_enable = (a.hasOwnProperty('inv_on_enable') && (a.inv_on_enable == true));
                return(btn);
            }).forEach((el) => {
...
get defaultPrefEditor(){
    ...
    div.innerHTML = `
        <fieldset style="display: grid; grid-template-columns: 1fr 1fr 1fr 1fr; text-align: left;">
            ...
            ${
                that.columns.sort((a,b) => {return(
                    (that.sort_col_selector_alpha == true)?(`${a.name}`.localeCompare(`${b.name}`)):(a.order - b.order)
                )}).map((a) =>{
...
/*
    getRows()
    just like get rows, except we're gonna return both the parsed dataset as well as the rowElement
    getSelected(), style
*/
getRows(){
    return(
...
addRow(row, renderCellsBool, defaultSortBool){
    ...
    return(new Promise((toot, boot) => {
        ...
        // 11/12/25 -- new hotness! the addRowCallback()
        Promise.resolve((that.addRowCallback instanceof Function)?that.addRowCallback(row):row).then((cData) => {

            let newRow = this.getRowElement(Object.assign({}, row, (cData instanceof Object)?cData:{}), (this.numRows + 1), (this.isNull(renderCellsBool) || (renderCellsBool === true)));
            ...
            that.log(`${that._className} v${that._version} | addRow() | addRowCallback() threw preventing row add | ${error}`);
            boot(error);
        });
    }));
}
    ...
    return(new Promise((toot, boot) =>{
        ...
        if (that.isNotNull(row)){
            new Promise((_t,_b) =>{
                ...
                /*
                    k ... we need to be able to update cell content with elements like on addRow
                    new hotness
                */
                let t = Object.assign({}, JSON.parse(row.DOMElement.dataset.rowdata));
                Object.keys(cData).forEach((colName) => {
                    if (cData[colName] instanceof Element){
                        row.DOMElement._elements[colName] = cData[colName];
                        t[colName] = cData[colName].textContent;
                    }else{
                        t[colName] = cData[colName];
                    }
                });
                row.DOMElement.dataset.rowdata = JSON.stringify(t);
...
updateRow(rowElement, data, enableCallbackBool){
    ...
    return(new Promise((toot, boot) =>{
        ...
        new Promise((_t,_b) => {
            ...
            // update the row and be out (old and busted)
            //rowElement.dataset.rowdata = JSON.stringify(Object.assign({}, JSON.parse(rowElement.dataset.rowdata), cData));

            // new hotness: update and be out (but allow Element values)
            let t = Object.assign({}, JSON.parse(rowElement.dataset.rowdata));
            Object.keys(cData).forEach((colName) => {
                if (cData[colName] instanceof Element){
                    rowElement._elements[colName] = cData[colName];
                    t[colName] = cData[colName].textContent;
                }else{
                    t[colName] = cData[colName];
                }
            });
            rowElement.dataset.rowdata = JSON.stringify(t);
...
clear(){
    ...
    requestAnimationFrame(() => {
        that.updateFooterMessage();
        that.dispatchEvent(new CustomEvent("table_cleared", {detail: {self: that}}));
    });
}
...
handleSearch(searchStr, selfReference){
    ...
    if (that.isNull(searchStr)){
        ...
        // gitrdun
        let rx = new RegExp(RegExp.escape(searchStr), 'i');
        that.rowElements.filter((el) =>{return(
