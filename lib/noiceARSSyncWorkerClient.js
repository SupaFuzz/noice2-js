            ...
            that.DBs[that.threadInfo.formDBMappings[args.schema].dbTagName].get({
                ...
                */
                ...
                if (that.isNotNull(dbRow)){
                    ...
                    that.dbInputFilter(args.schema, dbRow, row).then((dbRoow) => {
                        ...
                        if ((wqFields instanceof Object) && (Object.keys(wqFields).length > 0)){
                            ...
                            that.DBs.internalDB.put({
                                ...
                                that.DBs[that.threadInfo.formDBMappings[args.schema].dbTagName].put({
                                    ...
                                    // call dbInputNotifyCallback if we've got one
                                    if (that.dbInputNotifyCallback instanceof Function){ that.dbInputNotifyCallback({entryId: args.ticket, formName: args.schema}); }
                    ...
                    if ((wqFields instanceof Object) && (Object.keys(wqFields).length > 0)){
                        ...
                        that.DBs.internalDB.put({
                            ...
                            that.DBs[that.threadInfo.formDBMappings[args.schema].dbTagName].put({
                                ...
                                // call dbInputNotifyCallback if we've got one
                                if (that.dbInputNotifyCallback instanceof Function){ that.dbInputNotifyCallback({entryId: dbRoow.entryId, formName: args.schema}); }
...
filterRequestedFields(schema, fields, dbRow){
    ...
    if (
        ...
        (fields.length > 0) &&
        (fields.filter((a)=>{return(
            that.threadInfo.formDefinitions[schema].nameIndex.hasOwnProperty(a) ||
            dbRow.hasOwnProperty(a)
        ...
        */
        ...
        let match = fields.filter((a)=>{return(
            that.threadInfo.formDefinitions[schema].nameIndex.hasOwnProperty(a) ||
            dbRow.hasOwnProperty(a)
        )}).map((fieldName) => {return(
            that.threadInfo.formDefinitions[schema].nameIndex.hasOwnProperty(fieldName)?that.threadInfo.formIndexMappings[schema].hasOwnProperty(that.threadInfo.formDefinitions[schema].nameIndex[fieldName].id)?`${that.threadInfo.formIndexMappings[schema][that.threadInfo.formDefinitions[schema].nameIndex[fieldName].id]}`:`${that.threadInfo.formDefinitions[schema].nameIndex[fieldName].id}`:fieldName
        )});
...
/*
    getFieldLabelByID(schema, fieldID)
    return the label for the field identified by fieldID from the formDef
    if you've overrided the fieldID in an external callback you gotta deal
    with that over there
*/
getFieldLabelByID(schema, fid){
    let that = this;
    if (
        (that.isNotNull(schema)) &&
        (that.isNotNull(fid)) &&
        ...
        (that.threadInfo.formDefinitions[schema].idIndex instanceof Object)
            ...
            if (
                ...
                return(that.threadInfo.formDefinitions[schema].idIndex[fid].display_properties[
                    Object.keys(that.threadInfo.formDefinitions[schema].idIndex[fid].display_properties)[0]
                ].LABEL);
            }else{
                return(that.threadInfo.formDefinitions[schema].idIndex.hasOwnProperty(fid)?that.threadInfo.formDefinitions[schema].idIndex[fid].name:fid);
            }
    }else{
        that._app.log(`${that._className} v${that._version} | getFieldLabelByID(${schema}) | invalid input`)
        return(fid);
    }
}
...
/*
    getDBColName(schema, fieldName)
    return the indexedDB column name corresponding to the specified schema and fieldName
*/
getDBColName(schema, fieldName){
    if (
        (this.threadInfo instanceof Object) &&
        (this.threadInfo.formDefinitions instanceof Object) &&
        this.isNotNull(schema) &&
        (this.threadInfo.formDefinitions[schema] instanceof Object) &&
        (this.threadInfo.formDefinitions[schema].nameIndex instanceof Object) &&
        this.isNotNull(fieldName) &&
        (this.threadInfo.formDefinitions[schema].nameIndex[fieldName] instanceof Object) &&
        this.threadInfo.formDefinitions[schema].nameIndex[fieldName].hasOwnProperty('id')
    ){

        // holup, we need to respect the index mappings too
        return(
            (
                (this.threadInfo.formIndexMappings instanceof Object) &&
                (this.threadInfo.formIndexMappings[schema] instanceof Object) &&
                this.threadInfo.formIndexMappings[schema].hasOwnProperty(this.threadInfo.formDefinitions[schema].nameIndex[fieldName].id)
            )?this.threadInfo.formIndexMappings[schema][this.threadInfo.formDefinitions[schema].nameIndex[fieldName].id]:this.threadInfo.formDefinitions[schema].nameIndex[fieldName].id
        );

        //return(this.threadInfo.formDefinitions[schema].nameIndex[fieldName].id);
    }else{
        if (this.debug){ this._app.log(`${this._className} v${this._version} | getDBColName(${schema}, ${fieldName}) threw for bad input`); }
        throw(new noiceException({
            messageNumber: 300,
            messageType: 'internal',
            message: `invalid input`,
            thrownBy: `${this._className} v${this._version} | getDBColName()`,
            args: {schema: schema, fieldName: fieldName }
        }));
    }
}
...
/*
    getARSFieldName(schema, colName)
    inverse of getDBColName, give me the dbColName and the schema
    and I'll return unto you, the 'name' of the field from the
    formDef, or null if no match
*/
getARSFieldName(schema, colName){
    if (
        (this.threadInfo instanceof Object) &&
        ...
        this.isNotNull(schema) &&
        (this.threadInfo.formDefinitions[schema] instanceof Object) &&
        (this.threadInfo.formDefinitions[schema].idIndex instanceof Object) &&
        this.isNotNull(colName)
    ){

        return(
            (
                (this.threadInfo.formIDToIndexMappings instanceof Object) &&
                (this.threadInfo.formIDToIndexMappings[schema] instanceof Object) &&
                (this.threadInfo.formIDToIndexMappings[schema].hasOwnProperty(colName)) &&
                (this.threadInfo.formDefinitions[schema].idIndex[this.threadInfo.formIDToIndexMappings[schema][colName]] instanceof Object) &&
                this.threadInfo.formDefinitions[schema].idIndex[this.threadInfo.formIDToIndexMappings[schema][colName]].hasOwnProperty('name')
            )?this.threadInfo.formDefinitions[schema].idIndex[this.threadInfo.formIDToIndexMappings[schema][colName]].name:(
                /^_\d+$/.test(colName) &&
                (this.threadInfo.formDefinitions[schema].idIndex[`${colName}`.replace(/^_/, '')] instanceof Object) &&
                this.threadInfo.formDefinitions[schema].idIndex[`${colName}`.replace(/^_/, '')].hasOwnProperty('name')
            )?this.threadInfo.formDefinitions[schema].idIndex[`${colName}`.replace(/^_/, '')].name:(
                /^\d+$/.test(colName) &&
                (this.threadInfo.formDefinitions[schema].idIndex[colName] instanceof Object) &&
                this.threadInfo.formDefinitions[schema].idIndex[colName].hasOwnProperty('name')
            )?this.threadInfo.formDefinitions[schema].idIndex[colName].name:''
        )

    }else{
        if (this.debug){ this._app.log(`${this._className} v${this._version} | getARSFieldName(${schema}, ${colName}) threw for bad input`); }
        throw(new noiceException({
            messageNumber: 300,
            messageType: 'internal',
            message: `invalid input`,
            thrownBy: `${this._className} v${this._version} | getARSFieldName()`,
            args: {schema: schema, fieldName: colName }
        }));
    }
}
...
/*
    ...
    completeDBRow is optional, if given pass it to the callback so we can do logics on it
    for modify transactions, this is a copy of the row prior to modification where dbRow only
    contains the modified fields
*/
dbInputFilter(schema, dbRow, completeDBRow){
    let that = this;
    return(new Promise((toot, boot) => {
        if ((that.isNotNull(schema)) && (dbRow instanceof Object)){
            ...
            new Promise((_t,_b) => {
                _t((that.dbInputFilterCallback instanceof Function)?that.dbInputFilterCallback(schema, Object.assign({}, dbRow), that, (completeDBRow instanceof Object)?completeDBRow:{} ):Object.assign({}, dbRow));
            }).then((cData) => {
