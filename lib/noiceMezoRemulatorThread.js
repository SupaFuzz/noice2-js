...
/*
    ...
    this signalHadler will attempt to authenti
    cate
...
authenticateUser(data, evt){
    ...
    if (
        (data instanceof Object) && (
            (data.hasOwnProperty('user') && that.isNotNull(data.user)) ||
            (that.hasOwnProperty('user') && that.isNotNull(that.user))
        ) && (
            (data.hasOwnProperty('pass') && that.isNotNull(data.pass)) ||
            (that.hasOwnProperty('pass') && that.isNotNull(that.pass))
        )
    ){
        ...
        let user = (data.hasOwnProperty('user') && that.isNotNull(data.user))?data.user:(that.hasOwnProperty('user') && that.isNotNull(that.user))?that.user:null;
        let pass = (data.hasOwnProperty('pass') && that.isNotNull(data.pass))?data.pass:(that.hasOwnProperty('pass') && that.isNotNull(that.pass))?that.pass:null;
        ...
        if (that.networkErrorMonitor instanceof Function){ connectParams.fetchErrorMonitor  = that.networkErrorMonitor; };
        if (that.networkSuccessMonitor instanceof Function){ connectParams.fetchSuccessMonitor  = that.networkSuccessMonitor; };
        ...
        new noiceMezoRemulatorAPI(connectParams).authenticate({
            ...
            new Promise((_t,_b) => {_t(
                (that.authenticateUserCallback instanceof Function)?that.authenticateUserCallback(api, that, user, pass):false
            )}).then(() => {
                api.logout().then(() => {
                    that.signalParent({
                        type: 'authenticateUser',
                        data: { error: false, system: 'remulator', user: user }
                    });
                });
            }).catch((error) => {
                that.signalParent({
            ...
            /*
                here's the logic behind setting "netFail", which basically means to the app
                "the server can't be reached go into offline mode"

                * the server is not answering on the network
                  error.errorObject.messageNumber == 0 && error.errorObject.messageType = "network"

                * the server is horked in some way
                  error.errorObject.messageType = "server"

                * the user just entered the wrong password (remulator anyhow)
                  you're gonna get http status 403 (not a netfail just informational)
            */
                ...
                data: {
                    ...
                    netFail: (
                        (error instanceof Object) && (
                            (error.hasOwnProperty('_messageType') && (error._messageType == "non-ars")) ||
                            ((error.errorObject instanceof Object) && error.errorObject.hasOwnProperty('messageType') && (
                                (error.errorObject.messageType == "server") || (error.errorObject.messageType == "network")
                            ))
                        )
                    ),
...
mountInternalDB(){
    ...
    return(new Promise((toot, boot) => {
        ...
        if (that.internalDB instanceof noiceIndexedDB){
            if (that.debug){ that.log(`mountInternalDB(): already mounted returning existing instance`); }
            toot(that.internalDB);
        }else{
...
/*
    ...
    2) if we're online, fetch refreshed formDefinition for each and update meta table
        2.5) get the openAPI def from the postgrest interface too
    ...
    return (new Promise((toot, boot) => {
        ...
        if (navigator.onLine){
            if (
                ...
                // merge _openAPIDefinitions if we have them
                if ((! (that.api._openAPIDefinitions instanceof Object)) && (that._openAPIDefinitions instanceof Object)){
                    that.api._openAPIDefinitions = that._openAPIDefinitions;
                }
                toot(that.api);
            }else{
                ...
                new Promise((_t,_b) => {
                    ...
                    new Promise((_t,_b) => {
                        ...
                        new noiceMezoRemulatorAPI(connectParams).authenticate({
                            ...
                            // if we have _openAPIDefinitions cached merge it, else go get it
                            if (that._openAPIDefinitions instanceof Object){
                                api._openAPIDefinitions = that._openAPIDefinitions;
                                toot(api);
                            }else{
                                api.getOpenAPI({schema: 'remulator'}).catch((error) => {
                                    // error is nonfatal, just log it
                                    that.log(`getAPI() | (non-fatal) failed to pull remulator openAPI interface description: ${error}`);
                                }).then((ifDesc) => {
                                    if ((ifDesc instanceof Object) && (ifDesc.definitions instanceof Object)){
                                        that._openAPIDefinitions = ifDesc.definitions;
                                        api._openAPIDefinitions = that._openAPIDefinitions;
                                    }
                                    toot(api);
                                });
...
/*
    invokeMainLoop(initBool, throwOnErrorBool)
    ...
    if throwOnErrorBool is set, actually reject the promise if we get errors
    default is false. Most calls to this function expect it to resolve even in
    the case of an error. Others, such as syncAllNow() need to be aware if there
    was a failure
*/
invokeMainLoop(initBool, throwOnErrorBool){
    ...
    return (new Promise((toot, boot) => {
        that.awaitThreadLocks(['initializeThread']).catch((error) => {
            ...
            let syncFail = false;
            let syncFailError = null;
            that.threadLocks[fn] = new Promise((_t, _b) => {
                if (that.debug){ that.log(`${fn}() | start`); }
                that.syncing = true;
                that.syncAllForms(initBool, throwOnErrorBool).then(() => { _t(true); }).catch((error) => { _b(error); });
                ...
                new Promise((_t) => {_t(
                    ...
                    if (syncFail && (throwOnErrorBool === true)){
                        // do some bootin' here
                        boot(syncFailError);
                    }else{
                        if ((that.mainLoopSuccessCallback instanceof Function) && (! syncFail)){
                            that.mainLoopSuccessCallback().catch((error) => {
                                that.log(`${fn}() | mainLoopSuccessCallback() threw unexpectedly (ignored): ${error}`)
                            }).then(() => {
...
/*
    syncAllForms(initBool, throwOnErrorBool)
        ...
        that.getAPI().then((api) => {
            ...
            function recursor(idx){
                if (idx == queue.length){
                    ...
                    if ((throwOnErrorBool == true) && (errorForms.length > 0)){
                        boot(`forms failed sync: ${errorForms.join(", ")}`);
                    }else{
                        toot(true);
                    }
                }else{
                    that.syncForm(queue[idx], api, false, false, (initBool === true)).catch((error) => {
                        that.log(`${fn} ${(throwOnErrorBool == true)?'':' | ignored'} | ${queue[idx]} failed sync (${error})`);
                        errorForms.push(queue[idx]);
                    }).then(() => {
                        recursor(idx + 1);
                    });
                }
                            ...
                            that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName]._bulkSync.parseDataFile(result, formMeta, that).then((parsed) => {
                                ...
                                Promise.all(chunks.map((chunk, idx) => {return(new Promise((_t,_b) => {
                                    that.DBs[formMeta.dbTagName].bulkPut({
                                        ...
                                        objects: chunk.map((bRow) => {return(
                                            // pass the rows through rowWriteFilter if one exists in the config for this form
                                            (that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName].rowWriteFilter instanceof Function)?that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName].rowWriteFilter(bRow, formMeta):bRow
                                        )})
                                    }).then(() => {
                                    ...
                                    */
                                    that.writeFormMeta(formMeta, {
                                        ...
                                        toot({
                                            maxModifiedDate: (mmd > parsed.maxModifiedDate)?mmd:parsed.maxModifiedDate,
                                            entryIdIndex: entryIdIndex,
                                            rowUpdates: rowUpdates
                                        });
                                    });
...
dataStoreFilter(formMeta, apiRow){
    ...
    Object.keys(apiRow).forEach((fieldName) => {
        // new hotness supports selected db cols not mapped to an emulated form field
        if (formMeta.formDef.nameIndex[fieldName] instanceof Object){
            ...
            out[fieldName] = apiRow[fieldName];
        }
        ...
        new Promise((__t, __b) => {
            ...
            that.DBs[formMeta.dbTagName].deleteObject({
                ...
                that.DBs[formMeta.dbTagName].put({
                    storeName: formMeta.storeName,
                    object: (that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName].rowWriteFilter instanceof Function)?that.config.DBs[formMeta.dbTagName].storeDefinitions[formMeta.storeName].rowWriteFilter(dbRow, formMeta):dbRow
                }).then(() => {
                    __t(true);
                }).catch((error) => {
                    writeFail = true;
            ...
            Promise.all(
                    ...
                    that.handleAPIWrite(formMeta, api, trans).then((dbRow) => {
                        ...
                        */
                        ...
                        if (
                            ...
                            apiErrorCounter ++;
                            that.log(`${fn} | handleAPIWrite(${formMeta.formName}) | non-object error (looks serious) | ${error}`);
                            that.statusUpdate({
                                message: `sync: ${that.getSyncName(formMeta.formName)}`,
                                detail: `write (${numWritesComplete}/${writeQueue.length})`,
                                additionalDetail: '',
                ...
                if (
                    ...
                    that.statusUpdate({
                        ...
                        detail: `all api write transactions failed: see thread log`,
                        runAnimation: false,
                        error: true,
                        functionName: fn
                    });
...
syncAllNowSignalHandler(data, signalEvent){
    ...
    that.invokeMainLoop(false, true).catch((error) => {
