...
set element(el){ if (el instanceof noiceAutonomousCustomElement){
    ...
    if (el instanceof wcScreen){
        if (that.gainFocusCallback instanceof Function){ el.gainFocusCallback = async (fa, slf) => { return(that.gainFocusCallback(fa, slf)); } }
        el.addEventListener('gain_focus', (evt) => { if (that.gainedFocusCallback instanceof Function){ that.gainedFocusCallback(evt); } });
        if (that.loseFocusCallback instanceof Function){ el.loseFocusCallback = async (fa, slf) => { return(that.loseFocusCallback(fa, slf)); } };
