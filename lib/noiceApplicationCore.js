...
constructor(args, defaults, callback){
    ...
    if ((that.threads.hasOwnProperty('serviceWorker')) && (that.enableServiceworker === true) && ('serviceWorker' in navigator)){
        ...
        // new hotness -- spin up the serviceWorker!
        navigator.serviceWorker.register(that.threads.serviceWorker, {scope: './'}).then((registration) => {

            if (that.debug){ that.log(`Service Worker registered with scope: ${registration.scope}`); }
            ...
            /*
                appUpdatePendingCallback(registration, self)
                this fires when a new version of the serviceWorker is found on the server,
                downloaded and installing. Use this to activate "restart for update" type buttons
            */
            registration.addEventListener('updatefound', () => {
                const installingWorker = registration.installing;
                installingWorker.addEventListener('statechange', () => {
                    if (installingWorker.state === 'installed' && navigator.serviceWorker.controller) {
                        if (that.appUpdatePendingCallback instanceof Function){ that.appUpdatePendingCallback(registration, that); }
                    }
                });
            });
        ...
        /*
            setup a timer to check for updates (if we've got app_update_check_interval)
        */
        if (that.hasOwnProperty('app_update_check_interval') && (! isNaN(parseInt(that.app_update_check_interval)))){
            that.app_update_check_interval_timer = setInterval(() => {
                navigator.serviceWorker.ready.then(registration => {
                    registration.update().then((newRegistration) => {
                        // nothing else to do here, the 'statechange' handler above will handle the rest
                        if (that.debug){ that.log("[service worker] registration update requested (automatically)"); }
                    });
                });
            }, parseInt(that.app_update_check_interval))
        }
    }
} // end constructor
...
/*
    checkForAppUpdates()
    if we have a serviceWorker, this will call .update() on the registration
    if there's actually an update, the appUpdatePendingCallback
*/
checkForAppUpdates(){
    let that = this;
    if ((that.threads.hasOwnProperty('serviceWorker')) && (that.enableServiceworker === true) && ('serviceWorker' in navigator)){
        navigator.serviceWorker.ready.then(registration => {
            registration.update().then((newRegistration) => {
                // nothing else to do here, the 'statechange' handler above will handle the rest
                if (that.debug){ that.log("[service worker] registration update requested (user check)"); }
            });
        });
    }
}
...
const identifyClient = noiceApplicationCore.prototype.identifyClient;
export { noiceLogMessage, noiceLog, noiceApplicationCore, identifyClient }
